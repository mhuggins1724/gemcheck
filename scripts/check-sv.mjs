...
var sets = await s.from("sets").select("*").eq("era", "Scarlet & Violet").order("sort_order");
...
sets.data.forEach(function(x) {
  console.log(x.code + " | " + x.name + " | " + (countMap[x.code] || 0) + " cards | logo: " + (x.logo_url ? "YES" : "NO") + " | sort: " + x.sort_order);
});
