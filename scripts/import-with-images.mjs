...
async function importSet(setId) {
  ...
  for (var summary of set.cards) {
    ...
    var imageUrl = card.image ? card.image + "/high.webp" : "";
...
async function main() {
  console.log("Starting import WITH IMAGES for " + setsToImport.length + " sets...\n");
  ...
  for (var i = 0; i < setsToImport.length; i++) {
    ...
    var imagesInSet = cards.filter(function(c) { return c.image_url !== ""; }).length;
    totalImages += imagesInSet;
    ...
    console.log("  Uploaded " + cards.length + " cards (" + imagesInSet + " with images) | Total: " + totalUploaded);
  }

  console.log("\n\nDONE! Total cards: " + totalUploaded + " | Cards with images: " + totalImages);
}
